		...
		while (*curr != 0) {
			...
			if (v < (*curr)->getValue()) {
				...
				if (cn && cn->getBalance() > 0) {
					cout << "Вращаю правый, затем левый" << endl;
					rightLeft = true;
				}
				curr = &((*curr)->getLeftChild());
			} else if (v > (*curr)->getValue()) {
				...
				if (cn && cn->getBalance() < 0) {
					cout << "Вращаю левый, затем правый" << endl;
					leftRight = true;
				}
				curr = &((*curr)->getRightChild());
			}
...
void AVLTree<T>::rightRotation(Node<T>* cn, Node<T>** parent) {	
	cout << "Правое вращение: " << cn->getValue() << endl;
...
void AVLTree<T>::leftRotation(Node<T>* cn, Node<T>** parent) {
	cout << "Левое вращение: " << cn->getValue() << endl;
...
void AVLTree<T>::traversalPrintPost(Node<T>* n) {
	if(n != 0) {
		...
		std::cout << "Значение: " << n->getValue() << std::endl;
		std::cout << "Балансировка: " << n->getBalance() << endl;		
	}
}
